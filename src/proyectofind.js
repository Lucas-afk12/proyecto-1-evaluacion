...
/*Filtra las plantas que no empiezen por la letra "G" y que no tengan un precio menor o igual a 15 y que no sean de predominancia semi(hibrida)*/
 ...
 db.cultivo.find(
  {$and:[{planta:{$not:{$regex:"^G.*"}}},
         {$nor:[{precio:{$lte:15}},
                {"genetica.Predominancia":{$eq:"semi"}}]}]});
...
/*Filtra las plantas sativas con thc mayor al 15% con cbd none cosechadas antes de mayo y ordenadas por fecha*/
 ...
 db.cultivo.find(
  {$and:[{"genetica.tipo":{$eq:"sativa"}},
         {thc:{$gt:15}},
         {cosecha:{$lt: new Date("2020,5,0")}}]}).sort({cosecha:1});
...
db.cultivo.find(
 {$and:[{cbd:{$in:["alto","bajo"]}},
        {cosecha:{$lt:new Date("2020,9,0")}},
        {cosecha:{$gt:new Date("2020,8,0")}}]});
